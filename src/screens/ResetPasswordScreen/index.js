import { Text, View, StyleSheet, Linking, TouchableWithoutFeedback, Keyboard } from "react-native";
...
function ResetPasswordScreen() {
  ...
  const PasswordResetSendApi = async (email) => {
    setLoading({ ...loading, vis: true });
    let response = await Network("SHIFIR_UNTUSH", "post", {}, { email: email }, true);
    ...
    setTimeout(
      () => {
        setModalInfo({ vis: true, buttonName: Translation("open_email"), content: Translation("check_email"), type: "success" });
      },
      isIos ? 500 : 0
    );
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          E-posta adresinizi girdikten ve gönder düğmesine tıkladıktan sonra, e-postanıza erişim sağlayabileceğiniz ve şifrenizi
          sıfırlayabileceğiniz bir bağlantı gönderilecektir. Bu bağlantıyı tıklayarak yeni bir şifre oluşturabilirsiniz.
        </Text>
        ...
        <Formik
          ...
          onSubmit={(values) => {
            PasswordResetSendApi(values.email);
          }}
        >
